...
export const metadata: Metadata = {
  title: "Research",
  description:
    "Research at IEM, RVCE — 202 journal articles, h-index 8, sponsored funding, patents, and 8 research themes spanning OR, supply chain, manufacturing, quality, analytics, and sustainability.",
...
export default function ResearchPage() {
  ...
  // Amounts are stored as display strings ("₹20,00,000") — strip to digits to total them.
  const totalFunding = fundedProjects.reduce(
    (n, p) => n + Number(p.amount.replace(/\D/g, "")),
    0
  );
  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
        ...
        <section className="mb-14">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {[
              ...
              { label: "Sponsored Funding", value: `₹${fundingLakh}L` },
            ].map((m) => (
              <div
                key={m.label}
        ...
        {/* Sponsored Projects — funding highlight */}
        <section className="mb-14">
          <h2 className="text-2xl font-bold text-primary mb-6">
            Sponsored Research &amp; Consultancy
          </h2>
          <div className="bg-primary rounded-2xl p-6 sm:p-8 text-white">
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2 pb-6 border-b border-white/15">
              <div>
                <p className="text-4xl sm:text-5xl font-bold tracking-tight tabular-nums">
                  ₹{totalFunding.toLocaleString("en-IN")}
                </p>
                <p className="text-sm text-gray-200 mt-2">
                  Total sanctioned funding across {fundedProjects.length}{" "}
                  sponsored projects
                </p>
              </div>
              <p className="text-xs text-gray-300">
                Industry &amp; institutional grants
              </p>
            </div>
        ...
        <section className="mb-14">
          <div className="flex flex-wrap items-center gap-3 mb-2">
            ...
            <span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">
              {publications.length} selected papers · 2025–2026
            </span>
          </div>
        ...
        <section className="mb-14">
          <div className="flex flex-wrap items-center gap-3 mb-6">
            <h2 className="text-2xl font-bold text-primary">Patents</h2>
            <span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">
              {patents.length} filed ·{" "}
              {patents.filter((p) => p.status.includes("Granted")).length}{" "}
              granted
            </span>
          </div>
          <div className="space-y-3">
